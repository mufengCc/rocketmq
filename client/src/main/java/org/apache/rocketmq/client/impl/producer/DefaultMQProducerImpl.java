...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 如果是第一次启动,则调用 registerProducer 注册生产者,如果已经注册过,则不调用
                // 将数据存在 ConcurrentMap<String, MQProducerInner> producerTable 中
                // 使用
                /**
                 场景1：
    ...
    private SendResult sendDefaultImpl(
        ...
        // 1.根据发送消息的topic主题，从 topicPublishInfoTable 中获取对应的路由信息。 如果 topicPublishInfoTable 没有值，则从namseServer中同步
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 2.获取发送失败重试次数，当是同步发送时，重试2次.总共发送3次。第一次发送失败，则重试2次
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 5.更新发送延时故障机制。 如果开启，当发送到broker失败时，会记录一条数据，当下次发送消息时，调用方法选择队列时（selectOneMessageQueue（））尽量避开该broker
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                return null;
                            case SYNC:
                                // 当是同步消息发送时，且发送的结果不等于 SEND_OK，判断配置项，是否需要重发
